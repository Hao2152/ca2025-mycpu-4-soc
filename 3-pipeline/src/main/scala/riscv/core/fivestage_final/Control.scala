...
class Control extends Module {
  ...
  // A: We stall on load-use hazards because the load’s data isn’t produced until the MEM/WB stage, 
  //    so the next instruction would otherwise read a stale value; forwarding can’t shortcut this.
  // Hint: Consider data dependency and forwarding limitations
  //
  // Q2: What is the difference between "stall" and "flush" operations?
  // A: A stall freezes the PC and IF/ID registers so the current instruction waits for data, 
  //    whereas a flush wipes pipeline registers (inserts a NOP) to discard an incorrect or outdated instruction.
  // Hint: Compare their effects on pipeline registers and PC
  //
  // Q3: Why does jump instruction with register dependency need stall?
  // A: Jump/branch instructions need the latest rs1/rs2 value to compute their target; 
  //    if a prior instruction hasn’t written back yet, we must stall until a forwarding path can supply the fresh data.
  // Hint: When is jump target address available?
  //
  // Q4: In this design, why is branch penalty only 1 cycle instead of 2?
  // A: Simple branches are resolved in the ID stage, so only the IF stage’s next instruction is discarded, 
  //    yielding just a one-cycle penalty instead of two.
  ...
  // A: Without hazard detection, the CPU would read stale operands or continue down the wrong control path, 
  //    producing incorrect results or crashing almost immediately.
  // Hint: Consider data hazards and control flow correctness
  //
  // Q6: Complete the stall condition summary:
  // Stall is needed when:
  // 1. An EX-stage producer (load or jump source) targets the same register that the ID-stage instruction needs
  // 2. A MEM-stage load’s destination matches the ID-stage jump’s source register
  //
  // Flush is needed when:
  // 1. Flush when: A branch/jump is taken in ID, so the IF stage’s fetched instruction on the wrong path must be discarded
  //
}
